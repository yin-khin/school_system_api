...
const importData = async (req, res) => {
  ...
  const importRecordIfMissing = async (Model, item) => {
    const existing = item.id
      ? await Model.findByPk(item.id, { transaction })
      : null;
  ...
  try {
    ...
    if (data.sections?.length) {
      for (const item of data.sections) {
        if (await importRecordIfMissing(Section, item)) results.sections++;
      }
    }
    ...
    if (data.parents?.length) {
      for (const item of data.parents) {
        if (await importRecordIfMissing(Parent, item)) results.parents++;
      }
    }
    ...
    if (data.attendance?.length) {
      for (const item of data.attendance) {
        if (await importRecordIfMissing(Attendance, item)) results.attendance++;
      }
    }
    ...
    if (data.exams?.length) {
      for (const item of data.exams) {
        if (await importRecordIfMissing(Exam, item)) results.exams++;
      }
    }
    ...
    if (data.marks?.length) {
      for (const item of data.marks) {
        if (await importRecordIfMissing(Mark, item)) results.marks++;
      }
    }
    ...
    if (data.fees?.length) {
      for (const item of data.fees) {
        if (await importRecordIfMissing(Fee, item)) results.fees++;
      }
    }
    ...
    if (data.payments?.length) {
      for (const item of data.payments) {
        if (await importRecordIfMissing(Payment, item)) results.payments++;
      }
    }
    ...
    if (data.libraryTransactions?.length) {
      for (const item of data.libraryTransactions) {
        if (await importRecordIfMissing(LibraryTransaction, item))
    ...
    if (data.announcements?.length) {
      for (const item of data.announcements) {
        if (await importRecordIfMissing(Announcement, item))
    ...
    if (data.notifications?.length) {
      for (const item of data.notifications) {
        if (await importRecordIfMissing(Notification, item))
    ...
    if (data.timetables?.length) {
      for (const item of data.timetables) {
        if (await importRecordIfMissing(Timetable, item)) results.timetables++;
      }
    }
    ...
    if (data.assignments?.length) {
      for (const item of data.assignments) {
        if (await importRecordIfMissing(Assignment, item))
          results.assignments++;
      }
    }
